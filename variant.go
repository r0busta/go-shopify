...
type Variant struct {
	...
	CompareAtPrice       *EmptiableDecimal `json:"compare_at_price,omitempty"`
...
// EmptiableDecimal an emptiable decimal.Decimal
// When marshalled in JSON:
//  - marshalled into a nil, if a nil pointer
//  - marshalled into "", if the underlaying decimal value is zero
//  - marshalled as a normal decimal.Decimal, if the underlaying decimal is set to a non-zero value
type EmptiableDecimal struct {
	decimal.Decimal
}
